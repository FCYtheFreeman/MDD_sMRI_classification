...
def show_feature_map():
    cat = plt.imread('E:\python_project\cats_vs_dogs\data/test/9.jpg') #unit8
    plt.imshow(cat)
    cat = tf.cast(cat, tf.float32) #[360, 300, 3]
    x = tf.reshape(cat, [1, 480, 380,3]) #[1, 360, 300, 3]
    ...
    feature_map = tf.reshape(x_w, [480,380,out])
...
def show_rich_feature():
    ...
    with tf.variable_scope('conv1_1', reuse=tf.AUTO_REUSE):
        ...
        sess = tf.Session()
        sess.run(tf.global_variables_initializer())
        feature_map = tf.reshape(x_relu, [480,380,out])
        images = tf.image.convert_image_dtype (feature_map, dtype=tf.uint8)
        images = sess.run(images)
